...
!function (a) {
    ...
    c.prototype.initHeader = function () {
        ...
        a(this.$el).dragtable({
            ...
            beforeStop: function () {
                var d = [], e = [], f = [], g = [], h = -1, i = [];
                if (c.$header.find("th").each(function () {
                ...
                for (var j = 0; j < d.length; j++)h = a.fn.bootstrapTable.utils.getFieldIndex(c.columns, d[j]), -1 !== h && (f.push(c.columns[h]), c.columns.splice(h, 1));
                c.columns = c.columns.concat(f), b(), a.each(c.columns, function (a, b) {
                    var d = !1, e = b.field;
                    c.options.columns[0].filter(function (a) {
                        return d || a.field != e ? !0 : (i.push(a), d = !0, !1)
                    })
                }), c.options.columns[0] = i, c.header.fields = d, c.header.formatters = e, c.resetView(), c.trigger("reorder-column", d)
            }
        })
    }
}(jQuery);